...
export default function AdminHome() {
      ...
      headerRight: () => (
        <TouchableOpacity
          ...
          onPress={async () => {
            ...
            catch (e: any) { Alert.alert('Gagal logout', e?.message ?? 'Coba lagi.'); }
          }}
  ...
  React.useEffect(() => {
    ...
    const unsub = onValue(
      ...
      (err) => {
        // Abaikan error saat transisi logout
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <View className="flex-1 p-5 gap-4">
        {/* Card: CTA Tambah Kelas */}
        <View className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          <Text className="text-2xl font-bold mb-2">Dashboard Admin</Text>
          <Text className="text-slate-600 mb-6">
            Kelola kelas pilates: tambah kelas baru, atur slot, dan pantau pendaftar.
          </Text>
          <TouchableOpacity
            activeOpacity={0.75}
          ...
          >
            <Text className="text-white font-semibold">Tambah Kelas</Text>
          </TouchableOpacity>
        </View>

        {/* Daftar Kelas */}
        <View className="flex-1">
          <Text className="text-lg font-semibold mb-2">Kelas Terbaru</Text>
          ...
          {loading ? (
            ...
            <View className="bg-white rounded-2xl p-5 border border-slate-100 items-center">
              <Text className="text-slate-600">Belum ada kelas. Tambahkan kelas pertama kamu.</Text>
            </View>
          ) : (
                ...
                <View key={item.id} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                  <View className="flex-row items-start justify-between gap-3">
                    ...
                    <View className="items-end shrink-0 min-w-[120px]">
                      ...
                      <TouchableOpacity
                        activeOpacity={0.75}
                        onPress={() => router.push({ pathname: '/(admin)/classes/[id]', params: { id: item.id } })}
                        className="mt-2 px-3 h-9 rounded-xl items-center justify-center bg-gray-800"
                      >
